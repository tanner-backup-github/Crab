...
#define CRAB_OP(name, op)                                                      \
	void crab_##name(array *stack) {                                       \
		crab_value *o1 =                                               \
			GET_ARRAY(stack, stack->size - 1, crab_value *);       \
		crab_value *o2 =                                               \
			GET_ARRAY(stack, stack->size - 2, crab_value *);       \
		assert(o1->tag == VALUE_NUMBER && o2->tag == VALUE_NUMBER);    \
...
void print_parse_nodes(parse_node *node) {
	if (!node->children) {
		...
		for (size_t i = 0; i < node->children->size; ++i) {
			print_parse_nodes(
				GET_ARRAY(node->children, i, parse_node *));
		}
	}
}
	...
	if (!node->children) {
		...
		case LITERAL: {
			if (is_num(node_buf)) {
				...
				v->boolean = str_equals(node_buf, "true")
						     ? true
						     : false;
				v->tag = VALUE_BOOLEAN;
			} else {
		...
		if (str_equals(node_buf, "define")) {
			...
			for (size_t i = 1; i < node->children->size; ++i) {
				eval(GET_ARRAY(node->children, i, parse_node *),
				     stack, env);
			}
			...
			crab_value *to_assign =
				GET_ARRAY(stack, stack->size - 1, crab_value *);
			*v = *to_assign;
			if (to_assign->tag == VALUE_FUNCTION) {
				v->function.arg_names =
					malloc(to_assign->function.num_args *
					       sizeof(char *));

				for (size_t i = 0;
				     i < to_assign->function.num_args; ++i) {
					v->function.arg_names[i] =
						strdup(to_assign->function
							       .arg_names[i]);
				}
			}
			...
			eval(GET_ARRAY(node->children, 0, parse_node *), stack,
			     env);
			...
			if (b) {
				eval(GET_ARRAY(node->children, 1, parse_node *),
				     stack, env);
			} else {
				eval(GET_ARRAY(node->children, 2, parse_node *),
				     stack, env);
			}
		} else if (str_equals(node_buf, "lambda")) {
			assert(node->children->size == 2);

			crab_value *lambda = make_lambda_function(
				GET_ARRAY(node->children, 1, parse_node *),
				GET_ARRAY(node->children, 0, parse_node *));
			...
			if (str_equals(node_buf, "#ROOT#") &&
			    node->token.type == PUNCTUATION) {
				return;
			}

			crab_value *v =
				GET_HASH_TABLE(env, node_buf, crab_value *);
			size_t num_args = v->function.num_args;
			assert(node->children->size == num_args);
			...
			if (v->function.is_native) {
				...
				while (num_args) {
					...
					*local = *GET_ARRAY(stack,
							    stack->size - 1,
							    crab_value *);
					add_hash_table(
						env,
						v->function.arg_names[num_args -
								      1],
						local);
					remove_array(stack, stack->size - 1);
				...
				// @TODO: fix recursion!!!!!!
				for (size_t i = 0; i < v->function.num_args;
				     ++i) {
					remove_hash_table(
						env, v->function.arg_names[i]);
				}
			}
		}
	}
